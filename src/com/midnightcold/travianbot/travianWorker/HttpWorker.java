...
public class HttpWorker {
    ...
    private Cookie[] getCookieByHeaders(Header[] headers) {
        ...
        Object[] cookiesObject = cookiesList.toArray();
        Cookie[] localCookies = new Cookie[cookiesObject.length];
        
        for(int i=0; i<localCookies.length; i++)
            localCookies[i] = (Cookie) cookiesObject[i];
    ...
    private void updateCookie(Cookie[] responseCookies) {
        ...
        Object[] newCookiesObject = newCookiesList.toArray();
        Cookie[] newCookies = new Cookie[newCookiesObject.length];
        
        for(int i=0; i<newCookiesObject.length;i++)
            newCookies[i] = (Cookie) newCookiesObject[i];
        
        Cookie[] localCookies = new Cookie[newCookies.length+this.cookies.length];
        Logger.log.debug(localCookies.length+"");
        Logger.log.debug(newCookies.length+"");
        Logger.log.debug(this.cookies.length+"");
    ...
    protected void makeConnect(String method, String url, String messageBody) throws LoadHttpPageException {
        try {
            ...
            Object[] headersObject = gottenHeadsList.toArray();
            Header[] headers = new Header[headersObject.length];

            for (int i = 0; i < headers.length; i++) {

                headers[i] = (Header) headersObject[i];
            ...
            Logger.log.error("Ошибка вводы\\вывода при загрузке страницы" + ex.getMessage());
            throw new LoadHttpPageException();
        }
